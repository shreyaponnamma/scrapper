...
# Add the project root (sat-scrapper) to sys.path to allow imports from scripts.*
sys.path.append(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))

from scripts.utils.config import CEOS_RAW_EXCEL
...
async def get_mission_links(page):
    """
    Scrapes the CEOS mission index page for all mission links and details.
...
async def extract_field_value(page, label_text):
    ...
    Args:
        page (playwright.async_api.Page): The Playwright page object.
        label_text (str): The label text to look for in the DOM.
...
async def get_mission_details(browser, mission_url):
    """
    Navigates to a mission's detail page and extracts satellite and instrument information.
    
    Args:
        browser (playwright.async_api.Browser): The Playwright browser instance.
        mission_url (str): The URL of the mission to scrape.
        
    Returns:
        tuple: (details dictionary, list of instrument URLs) or (None, []) upon failure or exclusion.
    """
...
async def get_instrument_details(browser, instrument_url):
    """
    Navigates to an instrument's detail page and extracts technical specifications.
    
    Args:
        browser (playwright.async_api.Browser): The Playwright browser instance.
        instrument_url (str): The URL of the instrument to scrape.
        
    Returns:
        dict: A dictionary of instrument technical details or None if failed.
    """
    page = await browser.new_page()
    try:
...
async def main():
    """
    The main execution flow: launches browser, crawls all missions, filters them, 
    collects instrument data, and saves to an Excel file periodically and at the end.
    """
    async with async_playwright() as p:
        ...
        for i, m_link in enumerate(missions):
            ...
            if (i + 1) % 20 == 0:
                pd.DataFrame(all_data).to_excel(CEOS_RAW_EXCEL, index=False)
        pd.DataFrame(all_data).to_excel(CEOS_RAW_EXCEL, index=False)
        await browser.close()
