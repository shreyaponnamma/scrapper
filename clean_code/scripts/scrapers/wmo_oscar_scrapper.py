...
from scripts.utils.config import OSCAR_RAW_EXCEL
...
async def get_satellite_links(page):
    """
    Fetches the full list of satellite links from the WMO OSCAR satellites page.
    Uses persistent scrolling to ensure all satellites are captured from the dynamic table.
    
    Args:
        page: Playwright page object.
        
    Returns:
        list: A list of dictionaries containing 'acronym' and 'href' for each satellite.
    """
...
async def get_oscar_details(page, legend_text):
    """
    Robustly extracts all key-value pairs from a fieldset on an OSCAR page.
    Supports both standard table structures and fallback definition lists.
    
    Args:
        page: Playwright page object.
        legend_text (str): The text of the legend identifying the fieldset to scrape.
...
async def get_satellite_details(browser, sat_info):
    """
    Navigates to a satellite's detail page and extracts its metadata and instrument links.
    Applies status and launch date filters to exclude irrelevant satellites.
    
    Args:
        browser: Playwright browser object.
        sat_info (dict): Dictionary with satellite 'href' and 'acronym'.
        
    Returns:
        tuple: (details dictionary, list of instrument URLs) or (None, []) if filtered/error.
    """
...
async def get_instrument_details(browser, inst_url):
    """
    Navigates to an instrument's detail page and extracts basic metadata and detailed characteristics.
    
    Args:
        browser: Playwright browser object.
        inst_url (str): The URL of the instrument's detail page.
        
    Returns:
        tuple: (basic details dictionary, list of characteristic dictionaries).
    """
    page = await browser.new_page()
    try:
...
async def main():
    """
    Main execution function for the WMO OSCAR scraper.
    Coordinates link collection, concurrent detail extraction, and periodic data saving.
    """
